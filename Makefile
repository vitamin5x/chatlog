...
LDFLAGS := -ldflags '-X "github.com/vitamin5x/chatlog/pkg/version.Version=$(VERSION)" -w -s'
...
crossbuild-wechat-mem0-core:
	...
	for platform in $(PLATFORMS); do \
		...
		GOOS=$$os GOARCH=$$arch CGO_ENABLED=1 $(GO) build -trimpath $(LDFLAGS) -o $$output_name cmd/wechat-mem0-core/main.go || echo "Failed to build for $$os/$$arch"; \
	done
...
tag-delete:
	...
	@echo "Deleting tag $(TAG) locally and on $(REMOTE)..."
	git tag -d "$(TAG)"
...
tag-auto:
	...
	echo "Creating annotated tag $$new (from $$latest, level=$(TAG_LEVEL))..."; \
